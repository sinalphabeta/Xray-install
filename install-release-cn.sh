...
# This is a MODIFIED copy of the official script for machines in mainland China
# that cannot reach github.com / api.github.com directly:
#   - Version discovery and geodata are fetched through the jsDelivr mirror
#     (JSDmirror, https://cdn.jsdmirror.com), which serves repository files.
#   - Xray release binaries (zip + .dgst) are NOT available on jsDelivr-style
#     CDNs, so they are fetched through GitHub release proxies; several are
#     tried in order until the SHA256 verification passes.
# The original script is:
# https://github.com/XTLS/Xray-install/raw/main/install-release.sh

# You can override the mirrors right before running this script:
# export JSD_MIRROR='https://cdn.jsdmirror.com'
# export GH_PROXY='https://ghfast.top/'   # note the trailing slash

# Note: with the jsDelivr mirror the "latest" version is resolved from the
# newest git tag, so --beta behaves the same as a normal install here.
...
# jsDelivr mirror used for version discovery and geodata (repository files only)
JSD_MIRROR=${JSD_MIRROR:-https://cdn.jsdmirror.com}

# GitHub release-asset proxies, tried in order (the last empty entry means
# falling back to github.com directly). Override with the GH_PROXY variable.
if [[ -n "$GH_PROXY" ]]; then
  GH_PROXIES=("$GH_PROXY")
else
  GH_PROXIES=('https://ghfast.top/' 'https://ghproxy.net/' 'https://gh-proxy.com/' '')
fi
...
get_latest_version() {
  # Get Xray latest release version number through the jsDelivr mirror:
  # @latest resolves to the newest git tag, and core/core.go carries the
  # version number in the Version_x/Version_y/Version_z variables.
  local tmp_file
  tmp_file="$(mktemp)"
  local url="${JSD_MIRROR}/gh/XTLS/Xray-core@latest/core/core.go"
  if curl -x "${PROXY}" -sSfLo "$tmp_file" -H 'Cache-Control: no-cache' "$url"; then
    echo "get the latest version success"
  else
    "rm" "$tmp_file"
    echo "error: Failed to get the latest version from ${JSD_MIRROR}, please check your network."
    exit 1
  fi
  local ver_x ver_y ver_z
  ver_x="$(awk '/Version_x/ {print $NF; exit}' "$tmp_file")"
  ver_y="$(awk '/Version_y/ {print $NF; exit}' "$tmp_file")"
  ver_z="$(awk '/Version_z/ {print $NF; exit}' "$tmp_file")"
  "rm" "$tmp_file"
  if ! [[ "$ver_x" =~ ^[0-9]+$ && "$ver_y" =~ ^[0-9]+$ && "$ver_z" =~ ^[0-9]+$ ]]; then
    echo "error: Failed to parse the latest release version."
    exit 1
  fi
  RELEASE_LATEST="v${ver_x}.${ver_y}.${ver_z}"
  # The jsDelivr mirror cannot tell releases and pre-releases apart,
  # so --beta installs the same version as a normal install.
  PRE_RELEASE_LATEST="$RELEASE_LATEST"
}
...
download_xray() {
  local GITHUB_LINK="https://github.com/XTLS/Xray-core/releases/download/${INSTALL_VERSION}/Xray-linux-${MACHINE}.zip"
  local gh_proxy DOWNLOAD_LINK
  for gh_proxy in "${GH_PROXIES[@]}"; do
    DOWNLOAD_LINK="${gh_proxy}${GITHUB_LINK}"
    echo "Downloading Xray archive: $DOWNLOAD_LINK"
    if curl -f -x "${PROXY}" -R -H 'Cache-Control: no-cache' -o "$ZIP_FILE" "$DOWNLOAD_LINK"; then
      echo "ok."
    else
      echo 'warn: Download failed from this mirror, trying the next one.'
      continue
    fi
    ...
    if [[ "$CHECKSUM" != "$LOCALSUM" ]]; then
      echo 'warn: SHA256 check failed! Trying the next mirror.'
      continue
    fi
    return 0
  done
  echo 'error: All mirrors failed! Please check your network, or set GH_PROXY to a working mirror and try again.'
  return 1
}
...
install_geodata() {
  ...
  # The dat files are also published to the "release" branch of the repository,
  # so they can be fetched from the jsDelivr mirror directly.
  local download_link_geoip="${JSD_MIRROR}/gh/Loyalsoldier/v2ray-rules-dat@release/geoip.dat"
  local download_link_geosite="${JSD_MIRROR}/gh/Loyalsoldier/v2ray-rules-dat@release/geosite.dat"
